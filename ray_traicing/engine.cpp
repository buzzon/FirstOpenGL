...
void engine::display(void)
{
	...
	generateTexture();											// Генерируютекстуру

	glBegin(GL_QUADS);											// Рисую прямоуголькик заполняющий весь экран
		glTexCoord2f(0.0, 1.0); glVertex3f(-1.0, -1.0, 0.0);	// к нему применяю новоиспеченную текстуру
		glTexCoord2f(0.0, 0.0); glVertex3f(-1.0, 1.0, 0.0);		//
		glTexCoord2f(1.0, 0.0); glVertex3f(1.0, 1.0, 0.0);		//
		glTexCoord2f(1.0, 1.0); glVertex3f(1.0, -1.0, 0.0);		//
	glEnd();													//

	glFlush();													// Убуждаемся что буфер выведен
}
...
void engine::generateTexture()
{
	...
	build_and_render(framebuffer);     // Рендерю картинку и записываю её в framebuffer

	glGenTextures(1, &texName);																		  // Генерирую текстуру и назначаю её активной
