...
Vec3f alignment(Vec3f val)
{
	auto max = std::max(val[0], std::max(val[1], val[2]));
	if (max > 1) val = val * (1. / max);
	return val;
}
...
Vec3f cast_ray(	const Vec3f& orig, const Vec3f& dir, 
	...
	for (size_t i = 0; i < lights.size(); i++) {
		...
		//diffuse
		auto light = light_dir * N > 0 ? light_dir * N : 0;
		diffuse_light_intensity += lights[i].intensity * light;

		//specula
		auto specular = -reflect(-light_dir, N) * dir > 0 ? -reflect(-light_dir, N) * dir : 0;
		specular_light_intensity += powf(specular, material.specular_exponent) * lights[i].intensity;
	}

	auto pixel =	material.diffuse_color * diffuse_light_intensity * material.albedo[0] + 
					Vec3f(1., 1., 1.) * specular_light_intensity * material.albedo[1] + 
					reflect_color * material.albedo[2] + 
					refract_color * material.albedo[3];

	return alignment(pixel);
}
...
void ray_traicing(const std::vector<Sphere>& spheres, const std::vector<Light>& lights)
{
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			...
			auto cast = cast_ray(Vec3f(0, 0, 0), dir, spheres, lights);
